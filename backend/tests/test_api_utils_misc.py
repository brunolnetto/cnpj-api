from backend.app.api.utils.misc import (
    check_limit_and_offset, convert_to_bytes, MAX_LIMIT
)
...
def test_convert_to_bytes_invalid_format():
    assert convert_to_bytes("invalid_size") == None
    assert convert_to_bytes("10") == None  # Missing unit

def test_convert_to_bytes_empty_string():
    assert convert_to_bytes("") == None